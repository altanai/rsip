use rsip::{
    ...
    headers::typed::{tokenizers::UriWithParamsListTokenizer, CallInfo},
};
use std::convert::TryInto;
use testing_utils::Randomize;

validate_typed_header_trait!(CallInfo);
...
mod display {
    ...
    fn display1() {
        ...
        assert_eq!(
            CallInfo(uri_with_params_list.clone()).to_string(),
...
mod try_from_tokenizer {
    ...
    fn try_from_1() -> Result<(), rsip::Error> {
        ...
        assert_eq!(tokenizer.try_into(), Ok(CallInfo(uri_with_params_list)));
